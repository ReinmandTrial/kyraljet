<template>
  <UiPopup max-width="43rem" close-btn-mobile close-btn-desktop @close="$emit('close')">
    <div class="flex flex-auto flex-col lg:min-h-[36.25rem] lg:w-full lg:flex-row">
      ...
      >
        <h1 class="mb-5 text-center font-jost text-h3 lg:text-h4">Регистрация на сервисе</h1>
        <div class="mt-auto">
          <div class="mb-5">
            <UiButton class="w-full justify-center" @click="$emit('start-sign-up')">
              С помощью Email
            </UiButton>
          </div>
          <p class="text-center text-s text-gray-300">
            Регистрируясь, вы принимаете <span class="text-gray-400">условия сервиса</span>.
          </p>
        </div>
      </div>
    </div>
